...
>>
  ...
  (** The direct verification is harder ... 

<<  
  Lemma len_tr_Sk': 
    forall (m: list A) (k n: nat) (i: A), 
      len_tr_aux k m = k + n -> len_tr_aux k (i::m) = k + S n.
  Proof.
    intros * L.
    simpl.
    assert (U1: k + S n = S k + n) by omega; rewrite U1; clear U1. 
    revert L. 
    revert i k n.
  ...
  Proof.
    intros * L.
    unfold len_tr.
    rewrite <- Nat.add_0_l in L.
    apply (len_tr_Sk' m 0 n i) in L.
    omega. 
  Qed.
...
>>
  *)
  ...
  (*
  Lemma len_tr_GeGt_hd: 
    forall (m: list A) (n: nat) (i: A), 
      len_tr m >= n -> len_tr (i::m) > n.
  Proof.
    intros * L.
    apply len_tr_aux_GeGt_hd. 
  ...
  *)
  ...
  Lemma len_tr_GeGt: 
    forall (m: list A) (n: nat) (i: A), 
      len_tr m >= n -> len_tr (m ++ [i]) > n.
  Proof.
    induction m as [| x xs]; intros * H; simpl in *.
    - unfold len_tr in *;
      unfold len_tr_aux in *.
      omega. 
    - unfold 
